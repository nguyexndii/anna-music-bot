...
function createQueueEmbed(queue, page = 1) {
  ...
  if (is247) {
    ...
    embed.addFields({
      name: `${EMOJI_TAG.play} Đang phát hiện tại:`,
      value: `${curLink} | \`${queue.currentSong.duration || '??:??'}\``,
      inline: false
    });
  }
  ...
  if (userSongs.length === 0) {
    ...
    listSlice.forEach((s, idx) => {
      ...
      if (s.requestedById) {
        req = `• <@${s.requestedById}>`;
      } else if (s.requestedBy && s.requestedBy !== 'Auto' && s.requestedBy !== 'Auto (24/7)') {
        const cleanReq = String(s.requestedBy).replace(/🌐/g, '').trim();
      ...
      desc += `\`${realIndex}.\` ${titleMarkdown} | \`${s.duration || '??:??'}\` ${req}\n`;
    });
...
/**
 * Bộ nút chuyển trang cho hàng chờ (◀ Trước, Trang X/Y, Sau ▶, Xóa theo số...)
 */
function createQueuePaginationButtons(page = 1, totalPages = 1) {
  ...
  row.addComponents(
    ...
    new ButtonBuilder()
      ...
      .setDisabled(p >= total),
    new ButtonBuilder()
      .setCustomId('queue_delete_modal')
      .setLabel('Xóa theo số...')
...
/**
 * Menu Dropdown để xóa bài hát khỏi hàng chờ theo trang đang xem
 */
function createQueueDeleteSelectMenu(queue, page = 1, pageSize = 10) {
  ...
  const currentPage = Math.min(Math.max(1, page), totalPages);
  const startIndex = (currentPage - 1) * pageSize;
  const optionsSlice = userSongs.slice(startIndex, startIndex + pageSize);

  const selectMenu = new StringSelectMenuBuilder()
    .setCustomId(`menu_queue_remove_${currentPage}`)
    .setPlaceholder(`🗑️ Xóa bài ở Trang ${currentPage}/${totalPages} (hoặc Xóa tất cả)...`);
  ...
  // 2. Từng bài hát cụ thể trên trang hiện tại
  optionsSlice.forEach((song, idx) => {
    if (!song) return;
    const realIndex = startIndex + idx + 1;
    const songTitle = (song.title || 'Unknown').slice(0, 80);
    selectMenu.addOptions(
      new StringSelectMenuOptionBuilder()
        .setLabel(`${realIndex}. ${songTitle}`)
        .setDescription(`Thời lượng: ${song.duration || '??:??'} • Nhấn để xóa bài này`)
        .setValue(`remove_${realIndex - 1}`)
        .setEmoji(CUSTOM_EMOJIS.trash)
    );
  });
